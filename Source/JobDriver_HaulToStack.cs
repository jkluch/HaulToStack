...
namespace HaulToStack
{
    class JobDriver_HaulToStack : JobDriver
    {
        ...
        {
            IntVec3 destLoc = job.targetB.Cell;
            ...
            if (destName != null)
                return "ReportHaulingTo".Translate(hauledThing.LabelCap, destName);
            else
                return "ReportHauling".Translate(hauledThing.LabelCap);
        }
        ...
        public override bool TryMakePreToilReservations()
        {
            ...
            if ( job.targetA.Thing.def.stackLimit <=  1)
                return pawn.Reserve(job.GetTarget(StoreCellInd), job)
                    && pawn.Reserve(job.GetTarget(HaulableInd), job);
            else
                return pawn.Reserve(job.GetTarget(HaulableInd), job);
        }
